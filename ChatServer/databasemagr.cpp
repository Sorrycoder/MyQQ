...
bool DataBaseMagr::OpenDb(const QString &dataName)
{
    ...
    if (!userdb.open("root", "root")) {
        qDebug() << "Connection sql failed";
        // 说明数据不存在或链接失败
        return false;
    }
...
QJsonObject DataBaseMagr::ChangeUserPwd(const QString &name, const QString &old_passwd, const QString &new_passwd)
{
    QString strQuery = "SELECT id FROM USERINFO ";
    strQuery.append("WHERE name='");
    strQuery.append(name);
    ...
    if (query.next()) {
        nId     = query.value("id").toInt();
        // 查询到有该用户，继续修改密码
        QString updateQuery = "UPDATE USERINFO SET passwd='";
                    updateQuery.append(new_passwd);
                    updateQuery.append("'WHERE name='");
                    updateQuery.append(name);
                    updateQuery.append("' AND passwd='");
                    updateQuery.append(old_passwd);
                    updateQuery.append("';");
                    //qDebug()<<"updateQuery"<<updateQuery;
                    QSqlQuery updateQueryExec(updateQuery);
                    updateQueryExec.exec();
    }
    // 构建 Json 对象,把修改后的passwd传回去
    QJsonObject json;
    json.insert("id",  nId);
    json.insert("name", name);
    json.insert("passwd", new_passwd);
    return json;
}
...
//获取该群组下面的所所有用户,返回群里所有人状态信息
QJsonArray DataBaseMagr::GetGroupUsers(const int &groupId)
{
    QString strQuery = ("SELECT id,name FROM USERINFO WHERE ");//搜索群组里并且在线的人,没用到
