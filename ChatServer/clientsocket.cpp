...
{
    qDebug()<<"服务端void ClientSocket::SltReadyRead()";
    // 读取socket数据
    ...
    if (!doucment.isNull() && (jsonError.error == QJsonParseError::NoError)) {
        ...
        if (doucment.isObject()) {
            ...
            case GetFile:
            {
                emit signalDownloadFile(dataVal);
            }
                break;
...
void ClientSocket::ParseChangePasswd(const QJsonValue &dataVal)
{
    if (dataVal.isObject()) {
        ...
        QString strName = dataObj.value("name").toString();
        QString strPwd = dataObj.value("passwd").toString();
        QString strNewPwd=dataObj.value("newPassword").toString();
        QJsonObject jsonObj = DataBaseMagr::Instance()->ChangeUserPwd(strName, strPwd,strNewPwd);
        m_nId = jsonObj.value("id").toInt();
...
void ClientSocket::ParseUpdateUserHead(const QJsonValue &dataVal)
{
    ...
    if (dataVal.isObject()) {
        ...
        qDebug()<<"jsonObj:"<<jsonObj;
        qDebug()<<"jsonFriends:"<<jsonFriends;
...
void ClientSocket::ParseCreateGroup(const QJsonValue &dataVal)
{
    ...
    if (dataVal.isObject()) {
        ...
        int nId = dataObj.value("id").toInt();
        QString strName = dataObj.value("name").toString();
        QJsonObject json = DataBaseMagr::Instance()->CreateGroup(nId, strName);//存入数据库并返回群组信息
...
void ClientSocket::ParseGroupMessages(const QByteArray &reply)
{
    ...
    if (!doucment.isNull() && (jsonError.error == QJsonParseError::NoError)) {
        ...
        if (doucment.isObject()) {
            ...
            // 查询该群组里面的在线好友
            QJsonArray jsonArr = DataBaseMagr::Instance()->GetGroupUsers(nGroupId);//获取群组里所有人·
