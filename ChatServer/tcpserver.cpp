...
{
    ClientSocket *client = new ClientSocket(this, m_tcpServer->nextPendingConnection());//创建新的socket
...
{
    ClientSocket *client = (ClientSocket *)this->sender();//获取下显得客户端socket
    if (NULL == client) return;
    ...
    disconnect(client, SIGNAL(signalMsgToClient(quint8,int,QJsonValue)),this, SLOT(SltMsgToClient(quint8,int,QJsonValue)));
