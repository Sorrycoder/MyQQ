    ...
    QObject(parent)
{
    ...
    connect(m_tcpSocket, SIGNAL(readyRead()), this, SLOT(SltReadyRead()));//数据被写入socket后，异步读取数据
...
void ClientSocket::CheckConnected()
{
    if (m_tcpSocket->state() != QTcpSocket::ConnectedState)
    {
        ...
        qDebug()<<"上线ip"<<MyApp::m_strHostAddr;
...
void ClientSocket::SltSendMessage(const quint8 &type, const QJsonValue &dataVal)
{
    ...
    m_tcpSocket->write(document.toJson(QJsonDocument::Compact));//数据写到套接字里，write方法发送服务器里

    qDebug()<<"客户端发送请求："<<document;
}
...
{
    qDebug()<<"void ClientSocket::SltReadyRead()";
    // 读取socket数据
    QByteArray byRead = m_tcpSocket->readAll();
    //qDebug()<<"ClientSocket::SltReadyRead:"<<byRead;
    ...
    if (!doucment.isNull() && (jsonError.error == QJsonParseError::NoError)) {
        ...
        if (doucment.isObject()) {
            ...
            case Login://登录 17
            {
                ...
                break;
            case ChangePasswd://更改密码
            {
                ParseChangePasswd(dataVal);
                qDebug() << "修改密码" << dataVal;
            }
            case UserOnLine:
            {
                qDebug() << "user is oline" << dataVal;
                emit signalMessage(UserOnLine, dataVal);
            }
                break;
            case UserOffLine:
            {
                qDebug() << "user is offline" << dataVal;
                emit signalMessage(UserOffLine, dataVal);
            }
                break;
            case Logout:
            {
            ...
            case RefreshFriends:
            {
                emit signalMessage(RefreshFriends, dataVal);
            }
                break;
            case RefreshGroups:
            {
            ...
            case SendFile:
            {
                emit signalMessage(SendFile, dataVal);
            }
                break;
            case SendPicture:
            {
                emit signalMessage(SendPicture, dataVal);
            }
                break;
            case SendFace:
            {
...
void ClientSocket::ParseChangePasswd(const QJsonValue &dataVal)
{
    if (dataVal.isObject())
    {
        QJsonObject dataObj = dataVal.toObject();
        //qDebug()<<"ParseChangePasswd:"<<dataObj;
        if(m_nId == dataObj.value("id").toInt())
        {
            //qDebug()<<"m_nId:"<<m_nId;
            QString passwd=dataObj.value("passwd").toString();
            CMessageBox::Infomation(nullptr, QString("你的心密码为：%1").arg(passwd));
        }
    }
}
...
void ClientFileSocket::SltUpdateClientProgress(qint64 numBytes)
{
    ...
    // 发送进度信息
    emit signalUpdateProgress(bytesWritten, ullSendTotalBytes);
...
void ClientFileSocket::SltReadyRead()
{
    ...
    // 更新进度条
    emit signalUpdateProgress(bytesReceived, ullRecvTotalBytes);
    ...
    if ((bytesReceived >= ullRecvTotalBytes) && (0 != ullRecvTotalBytes))
    {
        ...
        emit signamFileRecvOk(Text, fileToRecv->fileName());
