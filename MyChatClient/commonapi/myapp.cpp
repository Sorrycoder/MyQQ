...
// 服务器相关配置
QString MyApp::m_strHostAddr        = "192.168.0.101";
...
QString MyApp::backgroundColor  ="#D3ECE4";
...
void MyApp::InitApp(const QString &appPath)
{
    ...
    m_strImagePath      = m_strDataPath + "Images/";
...
void MyApp::ReadSettingFile()
{
    ...
    settings.beginGroup("Server");
    m_strHostAddr = settings.value("HostAddr", "192.168.0.100").toString();
...
void MyApp::CheckDirs()
{
    ...
    // 表情目录
    dir.setPath(m_strImagePath);
    if (!dir.exists()) {
        dir.mkdir(m_strImagePath);
#ifdef Q_WS_QWS
        QProcess::execute("sync");
#endif
    }
