    ...
    ui(new Ui::ChatWindow)
{
    ...
    //改变颜色
    m_colorWidget=new ColorChoose(this);
    m_colorWidget->hide();

    //connect(m_colorWidget,SIGNAL(signalBackgroundColor(QString)),SLOT(SltBackgroundColor(QString)));
...
{
    //更新label
    m_cell = cell;//保存用户和好友的聊天信息
    ui->labelWinTitle->setText(QString("与 %1 聊天中").arg(cell->name));
    this->setWindowTitle(QString("与 %1 聊天中").arg(cell->name));

    m_nChatType = type;

    if (type == 0) {//默认为0，加载和此人聊天记录
        // 加载历史
        ui->widgetBubble->addItems(DataBaseMagr::Instance()->QueryHistory(m_cell->id, 10));//加载10条
...
void ChatWindow::UpdateUserStatus(const QJsonValue &dataVal)
{
    if (ui->tableViewGroups->isVisible()) {
        ...
        if (dataVal.isArray()) {
            ...
            for (int i = 1; i < nSize; ++i) {//从1开始，0是群号
...
void ChatWindow::on_btnSendMsg_clicked()
{
    ...
    // 构建json数据
    QJsonObject json;
    json.insert("id", MyApp::m_nId);//发送者
    json.insert("to", m_cell->id);//好友id
    json.insert("msg", text);
    json.insert("type", Text);

    // 发送消息
    emit signalSendMessage(0 == m_nChatType ? SendMsg : SendGroupMsg, json);
...
void ChatWindow::SltUpdateProgress(quint64 bytes, quint64 total)
{
   ...
   ui->widgetFileBoard->setVisible(bytes < total);//上传才会显示
...
void ChatWindow::SendFaceMsg(int faceIndex)
{
    ...
    // 发送消息
    emit signalSendMessage(SendFace, json);//槽mainwindow的SltSendMessage
    ...
    itemInfo->SetFace(faceIndex);
    itemInfo->SetOrientation(Right);
    itemInfo->SetMsgType(Face);//不设置默认是0，设置表情消息
...
void ChatWindow::on_btncolorSelect_clicked()
{
    QPoint pos= ui->btncolorSelect->pos();
     m_colorWidget->move(pos);
     connect(m_colorWidget,&ColorChoose::signalBackgroundColor,this,[=](QString color){
         ui->widgetBubble->setBackgroundColorPublic(color);
     });
     connect(m_colorWidget,&ColorChoose::signalLeftColor,this,[=](QString color){
         ui->widgetBubble->setLeftColorPublic(color);
     });
     connect(m_colorWidget,&ColorChoose::signalRightColor,this,[=](QString color){
         ui->widgetBubble->setRightColorPublic(color);
     });
     m_colorWidget->exec();
}

void ChatWindow::SltBackgroundColor(QString color)
{
//    ui->widgetBubble->setBackgroundColorPublic(color);
//    qDebug()<<"color:"<<color;
